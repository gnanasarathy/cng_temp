...
def get_LLM_response(query, task):
    llm = GoogleGenerativeAI(
        model="models/text-bison-001", google_api_key=os.environ["GOOGLE_API_KEY"]
    )
    examples = []
...
Stage 6: Onboarding → Play: Send onboarding documents after kick-off call, Make sure welcome email automation is sent.""",
    ...
    response = llm(
        new_prompt_template.format(template_user_input=query, template_task=task)
    )
...
st.set_page_config(
    page_title="Sales Wizard", layout="centered", initial_sidebar_state="collapsed"
)
